...
class PIDController(object):
    ...
    def __init__(self):
        ...
        self.desired_position = Position(0.0,0.0,0.3)
...
# TODO ADD YAW
    ...
    def step(self):
        ...
        '''
        if not self.position_control or self.position_control and self.pose_delta_time is not None:
            self.calc_error()
            if self.desired_velocity.magnitude() > 0:
                self.adjust_desired_velocity()
            return self.pid.step(self.pid_error)
        else:
...
#         self.current_calculated_z_velocity = (self.z - self.previous_z)/(self.pose_delta_time)
    def calc_error(self):
        ...
        self.pid_error = Error(self.velocity_error.x * - 10, self.velocity_error.y * - 10, dz * 10)
        if self.position_control:
            ...
            lr_step = self.lr_pid.step(self.position_error.x, self.pose_delta_time)
            # calculate a value to add to the velocity error based based on the
            # position error in the y (pitch) direction
            fb_step = self.fb_pid.step(self.position_error.y, self.pose_delta_time)
            # calculate a value to add to the velocity error based based on the
            # position error in the z (pitch) direction
            # ud_step = self.ud_pid.step(dz, self.pose_delta_time)

            # add the velocity values computed by the position error to their
            # error corresponding velocity error components
            self.pid_error.x += lr_step
            self.pid_error.y += fb_step
            # TODO TRY self.velocity_error.z += ud_step
