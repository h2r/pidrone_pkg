...
class LocalizationParticleFilter:
    ...
    def measurement_model(self, kp, des):
        ...
        for i in range(self.particles.num_particles):
            ...
            # There are three ways of selecting map kp and des to transform to:

            # NUMBER ONE gets the 9 map grids surrounding where we think the particle is
            sub_map_kp = []
            sub_map_des = []
            if grid_y > 0 and grid_x > 0 and grid_x < len(self.map_kp[0])-2 and grid_x < len(self.map_des[0])-2 and grid_y < len(self.map_des)-2 and grid_y < len(self.map_kp)-2:
            ...
            """
            # NUMBER TWO uses the single map grid where we think the particle is 
            ...
            pose = None
            if grid_y > 0 and grid_x > 0 and grid_x < len(self.map_kp[0]) and grid_x < len(self.map_des[0]) and grid_y < len(self.map_des) and grid_y < len(self.map_kp):
                m_kp, m_des = self.map_kp[grid_y][grid_x], self.map_des[grid_y][grid_x]
                if m_kp and m_des:
                    pose, num = self.compute_location(kp, des, m_kp, m_des)
