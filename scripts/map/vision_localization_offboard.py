...
Either writes pose data to a text file, or publishes image data to be
used in offboard computation
"""
...
class DataWriter(picamera.array.PiMotionAnalysis):
    ...
    def put_to_file(self, data):
        if self.lock:
            ...
            self.fp.write(repr(self.data))

            """
            # all this junk is necessary because will abbreviate lists with ellipses otherwise
            self.fp.write('[[') # write intiial brackets
            for i, lst in enumerate(self.data): # go through each camera frame
                if lst[0] is not None and lst[1] is not None: # make sure we have camera data
                    self.fp.write(str(lst[0])) # write the keypoint lists
                    self.fp.write(',[') # start of des lists
                    for j, l in enumerate(lst[1]): # loop through des lists
                        self.fp.write(repr(l)) 
                        if j < len(lst[1]) - 1: # don't write comma after last des list
                            self.fp.write(',')
                    self.fp.write('],') # end of des lists
                    self.fp.write(str(lst[2])) # write IR reading
                    self.fp.write(']') # end of this camera frame
                    if i < len(self.data) - 1: # don't write comma after last camera frame
                        self.fp.write(',')     
            self.fp.write(']]') # closing braces 
            """
