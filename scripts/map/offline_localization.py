...
runs localization for the PiDrone using an existing map file
"""
...
import ast
import string
from offline_localization_helper import *
...
class OfflineLocalizer(picamera.array.PiMotionAnalysis):
    ...
    def __init__(self, camera, bridge):
        ...
        # get rid of numpy words
        map_des = string.replace(map_des, ",dtype=uint8)", "")
        map_des = string.replace(map_des, "array(", "")

        # eval lists as python code
        map_kp = ast.literal_eval(map_kp)
        map_des = ast.literal_eval(map_des)
        fp.close()

        self.estimator = LocalizationParticleFilter()
        self.estimator.create_map(map_kp, map_des)
        print "Map Created from Text File"
