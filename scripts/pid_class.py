...
class PIDaxis():
    ...
    def step(self, err):
        time_elapsed = time.time() - self._t
        self._t = time.time()
        ...
        output = math.min(math.max(raw_output + midpoint, control_range[0]), control_range[1])

        return output

class PID:
    def __init__(self, 
        roll = PIDaxis(-2200.0, -0.5, -500000.0),
        pitch = PIDaxis(-2200.0, -0.5, -500000.0),
        yaw = PIDaxis(-1000000, 0,0),
        throttle = PIDaxis(10.0, 0.04, 700.0, kp_upper=0, d_cap=300)):
        self.roll = roll
        self.pitch = pitch
        self.yaw = yaw
    ...
    def calc_err(self, pos):
        ...
        _, _, pos_yaw = quat_to_rpy(pos)
        _, _, sp_yaw  = quat_to_rpy(self.sp)

        err_x = self.sp.pose.position.x - pos.pose.position.x
        err_x = self.sp.pose.position.y - pos.pose.position.y
        err_z = self.sp.pose.position.z - pos.pose.position.z
        err_yaw = self.sp_yaw - pos_yaw
        ...
        diff_angle = pos_yaw - err_angle # the difference between the drone's yaw and its sp_angle
