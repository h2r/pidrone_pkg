...
class AnalyzePhase(picamera.array.PiMotionAnalysis):
    def __init__(self, camera, bridge):
        ...
        self.map_detector = cv2.ORB(nfeatures=600, scoreType=cv2.ORB_FAST_SCORE)
        self.map = cv2.imread('map.jpg')
        self.map_kp, self.map_des = self.map_detector.detectAndCompute(self.map, None)
        ...
        if des1 is not None and des2 is not None:
            ...
            if len(good) > MIN_MATCH_COUNT:
                ...
                if transform is not None:
                    ...
                    pose = [transformed_center[0] + global_offset_x, transformed_center[1] + global_offset_y, yaw]
	...
	# start localization (without any particle filter)
        if self.locate_position:
            curr_kp, curr_des = self.detector.detectAndCompute(curr_img, None)
            self.pos = self.compute_location(curr_kp, curr_des, self.map_kp, self.map_des)
            print '--pose: ', self.pos[0], self.pos[1], self.pos[2]
...
def main():
    ...
    try:
        ...
        with picamera.PiCamera(framerate=90) as camera:
		    ...
		    print phase_analyzer.prev_img is not None and phase_analyzer.prev_time != last_time
