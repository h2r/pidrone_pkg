...
CAMERA_WIDTH = 320
CAMERA_HEIGHT = 240
#MAP_PIXEL_WIDTH = 2472  # in pixel
#MAP_PIXEL_HEIGHT = 924 
#MAP_REAL_WIDTH = 2.134  # in meter
#MAP_REAL_HEIGHT = 0.787
...
#
# This code has been refactored to take advantage of the speed of numpy arrays.
# Each particle holds an array with the entire set of weights and poses for
# all particles. Loops on the list of particles are replaced with operations
# on these sets of weights and poses.
#
...
class ParticleSet(object):
    def __init__(self, num_particles, poses):
        ...
        self.particles = [Particle(i, poses, self.weights) for i in xrange(num_particles)]
        self.poses = poses
...
class LocalizationParticleFilter:
    ...
    def sample_motion_model(self, x, y, yaw):
        ...
        # potential problem area that worked in testing
        for i in xrange(self.particles.num_particles):
    ...
    def measurement_model(self, kp, des):
        ...
        for i in xrange(self.particles.num_particles):
            ...
            self.particles.weights[i] = max(q, self.prob_threshold)
    ...
    def resample_particles(self):
        weights_sum = float(np.sum(self.particles.weights))
        new_poses = []
        new_weights = []

        normal_weights = self.particles.weights / weights_sum  # normalize
        samples = np.random.multinomial(self.particles.num_particles, normal_weights)  # sample
        for i, count in enumerate(samples):
            for _ in range(count):
                new_poses.append(self.particles.poses[i])
                new_weights.append(self.particles.weights[i])

        self.particles.poses = np.array(new_poses)  # replace poses
    ...
    def get_estimated_position(self):
        ...
        normal_weights = self.particles.weights / weights_sum  # get the probability
...
def create_map(file_name):
    ...
    detector = cv2.ORB(nfeatures=500, scoreType=cv2.ORB_FAST_SCORE, scaleFactor=1.3)
