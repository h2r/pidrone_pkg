...
# TODO if the velocity estimation is not good, we may use estimatedRigidTransform to update robot's position
...
class LocalizationParticleFilter:
    ...
    def sample_motion_model(self, ut, xt, delta_time):
        """
        ...
        # add noise
        noisy_ut = np.random.multivariate_normal(ut, self.covariance_motion)
        # noisy_ut = ut

        old_yaw = xt.position[3]
        xt.position[0] += (noisy_ut[0] * np.cos(old_yaw) + noisy_ut[1] * np.sin(old_yaw)) * delta_time
        xt.position[1] += (noisy_ut[0] * np.sin(old_yaw) + noisy_ut[1] * np.cos(old_yaw)) * delta_time
        xt.position[2] = self.z
        xt.position[3] += noisy_ut[3] * delta_time
        # the range is (-pi, pi]
    ...
    def update(self, z, angle_x, angle_y, ut, delta_time, kp, des):
        """
        ...
        for p in self.particles:
            ...
            self.sample_motion_model(ut, p, delta_time)
