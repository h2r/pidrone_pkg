...
# Uses homography to get the R and t
    def getRt(self, img2, img1 = None):
        ...
        if len(good) > self.min_match_count:
            ...
            # Find Homography
            H, mask = cv2.findHomography(src_pts, src_pts, cv2.RANSAC,5.0)
...
# Updates curr pos based on other params
    ...
    def get_pose(self, start_R, start_t, s):
        retval, Rs, ts, norms = cv2.decomposeHomographyMat(self.est_H, self.camera_matrix)
...
if __name__ == '__main__':
    ...
    pospub = rospy.Publisher('/pidrone/homo_est', PoseStamped, queue_size=1)
    ...
    for curr_img in camera.getImage():
        ...
        if cv2.waitKey(1) & 0xFF == ord('n'):
            ...
            if first_img is None: first_img = curr_img
                ...
                for i in range(4):
                    position = homography.get_pose(start_R, start_t, i)
