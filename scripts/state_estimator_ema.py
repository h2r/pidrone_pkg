...
# TODO modify value based on roll and pitch values
    def filter_range(self, range_reading):
        ''' Smoothe the range reading using and ema filter '''
        # the ema filter constant
        alpha = 0.2
        # store the current z position of the drone
        current_range = self.state.pose_with_covariance.pose.position.z
        # use an ema filter to smoothe the range reading
        smoothed_range = (1.0 - alpha) * range_reading + alpha * current_range
        # ensure that the range value is between 0 and 0.55 m
        smoothed_range = max(0, min(smoothed_range, 0.55))
        # update the current z position
        self.state.pose_with_covariance.pose.position.z = smoothed_range
